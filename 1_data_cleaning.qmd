...
df_casen <- df_casen |> 
  ...
  mutate(
    civilstatus = case_when(
      year == 2017 & civilstatus %in% 2:3 ~ 2, 
      year == 2017 & civilstatus %in% 4:6 ~ 3,
      year == 2017 & civilstatus == 7 ~ 4,
      year == 2017 & civilstatus == 8 ~ 5,
      TRUE ~ civilstatus
    )
  )
```
...
df_casen <- df_casen |> 
  ...
  mutate(
    ...
    across(
      .fns = ~ as.factor(.x), 
      .cols = c(region, urb, gender, student, civilstatus)
    ),
    # New level of factor
    # across(
